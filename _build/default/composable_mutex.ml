...
let lock m =
  let rec block r =
    ...
    match old with
    | Unlocked ->
        if Atomic.compare_and_set m old (Locked []) then Some ()
        else block r (* failed CAS; retry *)
    | Locked l ->
        if Atomic.compare_and_set m old (Locked (r :: l)) then None
        else block r (* failed CAS; retry *)
  in
...
let rec unlock m =
  ...
  | Locked [] ->
      if Atomic.compare_and_set m old Unlocked then ()
